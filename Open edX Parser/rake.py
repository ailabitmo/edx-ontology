...
def separate_words(text, min_word_return_size):
    ...
    for single_word in splitter.split(text):
        ...
        #print(single_word,current_word)
        #current_word=st.lemmatize(current_word)
...
class Rake(object):
    ...
    def run(self, text):
        ...
        condidat_terms = []
        good_terms = []
        for cond_terms in sorted_keywords:
            find=cond_terms[0]
            if  cond_terms[1]>1:
                #sparql_dbpedia = SPARQLWrapper()
                #sparql_dbpedia.setReturnFormat(HTML)
